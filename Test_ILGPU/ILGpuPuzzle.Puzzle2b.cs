...
public partial class ILGpuPuzzle : IDisposable
{
    public int[] Puzzle2b()
    {
		...
		// create the kernel, config not required because the kernel is auto grouped
		var kernel = _accelerator.LoadAutoGroupedStreamKernel<Index1D, ArrayView<int>, ArrayView<int>, ArrayView<int>>(sumTwoArrays);
        // execute the kernel
        kernel(bufferOut.IntExtent, bufferA.View, bufferB.View, bufferOut.View);
